...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.setWindowIcon(QIcon(get_image_path('icon.ico')))
        ...
        self.view_model.current_filename.observe(self.__update_title_bar)
        self.view_model.current_world_position.observe(self.__update_status_bar)
...
<<<<<<< HEAD
        ...
        edit_menu.addAction(action_detect)
...
>>>>>>> 8fcab17a001e8d8da21e572b3094a84a4c8dfe35
    ...
    def __setup_toolbar(self):
        ...
        **Show Growth Pattern : 현재 종양과 대조군의 시계열적 성장 패턴을 보여준다.
        ...
        action_new = QAction("New File", self, icon=QIcon(get_image_path('new.png')))
        action_new.triggered.connect(self.view_model.on_new_project_click)
        self.toolbar.addAction(action_new)

        action_open = QAction("Open File", self, icon=QIcon(get_image_path('open.png')))
        action_open.triggered.connect(self.view_model.on_open_project_click)
        self.toolbar.addAction(action_open)

        action_save = QAction("Save File", self, icon=QIcon(get_image_path('save.png')))
        action_save.triggered.connect(self.view_model.on_save_project_click)
        ...
        action_import = QAction("Import DICOM Folder", self, icon=QIcon(get_image_path('import.png')))
        action_import.triggered.connect(self.view_model.on_import_dicom_click)
        self.toolbar.addAction(action_import)

        action_delete_layer = QAction("Delete Layer", self, icon=QIcon(get_image_path('delete.png')))
        action_delete_layer.triggered.connect(self.view_model.on_delete_layer_click)
        self.toolbar.addAction(action_delete_layer)

        action_delete_series = QAction("Delete Series", self, icon=QIcon(get_image_path('delete_all.png')))
        action_delete_series.triggered.connect(self.view_model.on_delete_series_click)
        ...
        action_undo = QAction("Undo", self, icon=QIcon(get_image_path('undo.png')))
        action_undo.triggered.connect(self.view_model.on_undo_click)
        self.toolbar.addAction(action_undo)

        action_redo = QAction("Redo", self, icon=QIcon(get_image_path('redo.png')))
        action_redo.triggered.connect(self.view_model.on_redo_click)
        self.toolbar.addAction(action_redo)

        action_clear_path = QAction("Clear Path", self, icon=QIcon(get_image_path('clear.png')))
        ...
        action_reconstruct.setIcon(QIcon(get_image_path('render.png')))
        action_reconstruct.triggered.connect(self.view_model.on_reconstruct_click)
        self.toolbar.addAction(action_reconstruct)

        action_export = QAction("Export Tumor", self)
        action_export.setIcon(QIcon(get_image_path('export.png')))
        action_export.triggered.connect(self.view_model.on_export_model_click)
        ...
        action_growth.setIcon(QIcon(get_image_path('compare.png')))
        action_growth.triggered.connect(self.view_model.on_add_comparison_click)
        self.toolbar.addAction(action_growth)

        action_growth = QAction("Show Growth Pattern", self)
        action_growth.setIcon(QIcon(get_image_path('chart.png')))
        action_growth.triggered.connect(self.view_model.on_show_growth_pattern_click)
        self.toolbar.addAction(action_growth)

    def __update_status_bar(self, pos_world):
        """
        마우스의 위치에 대응되는 실세계 좌표를 메인 윈도우 하단의 status bar에 표시한다.
        ...
        if pos_world is not None:
            x, y, z = pos_world
            self.statusBar().showMessage(f'({int(x)}, {int(y)}, {int(z)}) [mm]')
    
    def __update_title_bar(self, filename):
        """
        메인 윈도우의 title bar에 현재 프로젝트 파일명을 표시한다.
